...
class TaskManager:
    """
    This class manages tasks and users for a task management application.

    It interacts with the FileManager class to read and write data from JSON files
    and provides methods for user management, task management, and authentication.
    """
    ...
    def __init__(self, tasks_file='tasks.json', users_file='users.json'):
        ...
        Args:
            tasks_file (str, optional): The filename for storing tasks data. Defaults to 'tasks.json'.
            users_file (str, optional): The filename for storing users data. Defaults to 'users.json'.
        """
        self.file_manager = FileManager(tasks_file, users_file)
        self.tasks = [self.dict_to_task(task) for task in self.file_manager.read_data(tasks_file)]
        self.users = self.file_manager.read_data(users_file)

    def dict_to_task(self, task_dict):
        """
        Converts a dictionary containing task data (id, name, start_time, end_time) to a Task object.
    ...
    def create_user(self, username, password, role='user'):
        """
        Creates a new user with the specified username, password, and role (admin or user).

        Args:
            username (str): The username for the new user.
            password (str): The password for the new user.
            role (str, optional): The role for the new user ('admin' or 'user'). Defaults to 'user'.

        Returns:
            bool: True if the user is created successfully, False otherwise (if username already exists).
        """
    ...
    def login(self, username, password):
        ...
        Returns:
            Admin/User/None: An Admin object if the user is admin, a User object otherwise, or None on failed login.
        """
    ...
    def update_user_details(self, old_username, new_username, new_password):
        ...
        Returns:
            bool: True if the user details are updated successfully, False otherwise (user not found).
        """
        for user in self.users:
    ...
    def delete_user(self, username):
        # This function allows admin to remove a user whose no long required or he/she is 
    ...
    def add_task(self, task):
        ...
        self.file_manager.write_data([task.__dict__ for task in self.tasks], self.file_manager.tasks_file)
        return True
